...
from purchase_api import (
    get_payment_methods, get_qris_code, purchase_package
)
...
def purchase_init(body: PurchaseInitRequest):
    return safe_call(purchase_package, _api_key(), body.tokens.model_dump(), body.package_option_code)

@app.post("/pay/methods")
def pay_methods(body: PaymentInit):
    return safe_call(get_payment_methods, _api_key(), body.tokens.model_dump(), body.token_confirmation, body.payment_target)
